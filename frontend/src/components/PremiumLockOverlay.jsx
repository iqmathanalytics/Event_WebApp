...
function LockPromoPanel({ variant = "deal", onClose }) {
  ...
  return (
    ...
    >
      <button
        type="button"
        onClick={onClose}
        className="absolute right-1.5 top-1.5 z-20 grid h-8 w-8 place-content-center rounded-full bg-black/25 text-white/90 backdrop-blur-sm transition hover:bg-black/40"
        aria-label="Close"
      >
      ...
      <div className="relative z-10 px-4 pb-6 pt-5 text-center">
        ...
        <div className="mt-1 flex items-center justify-center gap-1 text-[10px] font-medium text-amber-200/80">
          ...
          <span className="premium-lock-pulse-text">Sign in to unlock</span>
          <motion.span
            animate={{ y: [0, -3, 0] }}
...
function PremiumLockOverlay({ open, onClose, variant = "deal" }) {
  ...
  return createPortal(
    <AnimatePresence>
      {open ? (
        <motion.div
          key="premium-lock-root"
          role="dialog"
          aria-modal="true"
          aria-labelledby="premium-lock-heading"
          className="fixed inset-0 z-[230] flex items-center justify-center bg-slate-950/65 p-4 backdrop-blur-[3px]"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
